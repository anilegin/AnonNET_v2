...
from utils.scene_detection import clip_creation, get_scene_video_paths
...
def load_source_tensor_from_temp(scene_id, size, default_source=None):
    """
    Load 'source_images/source_<scene_id>.png' -> (1,3,size,size) in [-1,1], on cuda.
    If not found, optionally fallback to default_source path.
    """
    path = os.path.join("source_images", f"source_{scene_id}.png")
...
def main(args):
    ...
    scene_video_paths = get_scene_video_paths(args)
    ...
    with tqdm(total=total_frames, desc="Overall Progress") as pbar:
        while finished_tasks < len(async_results):
            while not progress_queue.empty():
                n = progress_queue.get()  # each .put(1) => 1 frame
                pbar.update(n)
