...
def load_source_tensor_from_temp(scene_id, size, default_source=None):
    """
    Load 'temp_source_images/source_<scene_id>.png' -> (1,3,size,size) in [-1,1], on cuda.
    If not found, optionally fallback to default_source path.
    """
    path = os.path.join("temp_source_images", f"source_{scene_id}.png")
...
def main(args):
    ...
    for i, scene_info in enumerate(scenes):
        ...
        if sid not in used_ids:
            used_ids.add(sid)
            source_jpg = os.path.join(temp_source_dir, f"source_{sid}.png")
