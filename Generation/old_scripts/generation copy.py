...
#  - video_utils: splitting, merging, extracting frames
#  - detect_scenes_and_assign_ids: scene detection
#  - networks.generator: your pretrained Generator
# ----------------------------------------------------------------------
...
def process_segment(gen, args, source_tensor, segment_path, save_path, progress_queue=None):
    ...
    # Possibly compute motion encoding
    if args.model == 'ted':
        h_start = None
    else:
...
def create_clip_tasks(scene_video_paths, clip_length):
    """
    For each scene_id,scene_video => split into 'clip_length'-second sub-videos.
    ...
    for scene_id, scene_video_path in scene_video_paths:
        ...
        # Split scene video into N clips of length 'clip_length'
...
if __name__ == "__main__":
    ...
    # 3) Extract sub-videos for each scene + first-frame source images
    ...
    # 5) Count total frames for all tasks -> for single tqdm bar
    total_frames = 0
    ...
    # 6) Run tasks in parallel, capturing progress
    manager = mp.Manager()
    ...
    # 7) Merge the processed clips in the same order as tasks
