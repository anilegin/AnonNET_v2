...
def create_clip_tasks(scene_video_paths, clip_length):
    """
    Given a list of scene videos (list of tuples: (scene_id, scene_video_path)),
    split each scene video into clips of length clip_length (in seconds) and return
    a list of tasks. Each task is a tuple: (clip_path, scene_id, output_clip_path).
    The tasks are ordered so that all clips from scene1 come before those from scene2, etc.
    """
    tasks = []
    for scene_id, scene_video_path in scene_video_paths:
        # Create a temporary directory for the clips for this scene.
        scene_clip_dir = os.path.join(os.path.dirname(scene_video_path), f"clips_scene_{scene_id}")
        os.makedirs(scene_clip_dir, exist_ok=True)
        # Use your video_utils.split_video_by_seconds to split into clip_length-second clips.
        # (If the scene video is shorter than clip_length, it will be left as a single clip.)
        video_utils.split_video_by_seconds(scene_video_path, clip_length, scene_clip_dir)
        # Collect clip file paths in order.
        clip_paths = sorted([os.path.join(scene_clip_dir, f) for f in os.listdir(scene_clip_dir) if f.endswith('.mp4')])
        for i, clip_path in enumerate(clip_paths):
            output_clip_path = os.path.join(scene_clip_dir, f"processed_clip_{i}.mp4")
            tasks.append((clip_path, scene_id, output_clip_path))
    return tasks
...
def process_clip_worker(task, args, gen):
    ...
    Loads the clip and the corresponding source image (via load_source_tensor_from_temp),
    then processes the clip using the model (via process_segment) and saves the output.
    Returns the output clip path.
    """
    clip_path, scene_id, output_clip_path = task
    # Load the appropriate source tensor for this scene.
    source_tensor = load_source_tensor_from_temp(scene_id, args)
    # Create a dummy progress queue (or simply ignore progress reporting).
    from multiprocessing import Queue
    progress_queue = Queue()
    # Process the clip (process_segment is assumed to work on clips of any length).
...
if __name__ == "__main__":
    ...
    # No source_path argument here because source images are created dynamically.
    ...
    # New parameter: clip_length in seconds (we use 20 seconds per your request)
    parser.add_argument("--clip_length", type=int, default=20, help="Clip length in seconds for each scene")
    ...
    # Create temporary directories for scene videos and source images.
    ...
    # For each scene, extract its subvideo and, for the first occurrence of each scene_id,
    # extract the first frame and save it as the source image.
    ...
    for idx, scene in enumerate(scenes):
        ...
        # Extract subvideo corresponding to scene [start, end].
        ...
        # For the first occurrence of a scene_id, extract and save the first frame.
    ...
    # Now, for each scene video, split it into 20-second clips.
    tasks = create_clip_tasks(scene_video_paths, args.clip_length)
    print(f"Created {len(tasks)} clip tasks.")

    # Process these tasks using multiprocessing (4 workers).
    pool = mp.Pool(processes=4)
    process_func = partial(process_clip_worker, args=args, gen=gen)
    processed_clip_paths = pool.map(process_func, tasks)
    pool.close()
    pool.join()
    ...
    # Merge the processed clips (in order) into the final output video.
