...
celeba_female_train = '/data/stars/share/celeba_hq/train/female'
celeba_female_val = '/data/stars/share/celeba_hq/val/female'
# output_female = '/data/stars/share/celeba_hq/female_anon'
output_female = '/data/stars/share/celeba_hq/female_anon'
...
def filter_processed_videos(video_paths, output_folder):
    """
    ...
    for vid_path in video_paths:
        ...
        pattern = os.path.join(output_folder, f"*{video_name}*_anon_*.png")
...
def main(args):
    ...
    all_videos = get_all_videos(celeba_female_train)
    all_videos.extend(get_all_videos(celeba_female_val))
    
    # if args.even == 'even':
    #     all_videos = all_videos[1::2]
    # elif args.even == 'odd':
    #     all_videos = all_videos[0::2]
    if args.even == 1:
        all_videos = all_videos[0::4]
    elif args.even == 2:
        all_videos = all_videos[1::4]
    elif args.even == 3:
        all_videos = all_videos[2::4]
    elif args.even == 4:
        all_videos = all_videos[3::4]
    
    valid_paths_list = filter_processed_videos(all_videos, output_female)
    ...
    # ---------------------------------------------------
    log_file = os.path.join('/home/aegin/projects/anonymization/AnonNET/stats_and_jobs_celeba_woman', f"processing_summary_female_celeba_{args.even}.txt")
    ...
    for vid_path in tqdm(valid_paths_list, desc="Anon_video"):
        try:
            ...
            save_p = os.path.join(output_female, f"{video_name}_anon_{filename_info}.png")
...
#     """
#     Main function that:
#       1) Gathers .mp4 files in `celebvhq_train`
#       2) Splits them by a batch range (e.g., "0-300")
#       3) For each video, checks if an anonymized image exists
#       4) If not, extracts a frontal frame, detects face, anonymizes, saves .png
#     """

#     # ---------------------------------------------------
#     # 2) Gather all .mp4 videos and slice by batch range
#     # ---------------------------------------------------
#     all_videos = get_all_videos(celeba_female_train)
#     all_videos = all_videos.extend(get_all_videos(celeba_female_val))
...
#     all_videos = filter_processed_videos(all_videos, output_female)
#     # start_str, end_str = args.batch.split("-")
#     # start_val, end_val = int(start_str), int(end_str)
    
#     # valid_paths_list = all_videos[start_val:end_val]
...
#         # Extract video base name, e.g. "example.mp4" -> "example"
#         base_name = os.path.basename(vid_path)      # e.g. "example.mp4"
#         video_name = os.path.splitext(base_name)[0] # e.g. "example"
...
#         # 4.2) Detect face & create segmentation masks
...
#         if len(outputs)==0:
#             print(f"No faces found in {vid_path} (or detection failed). Skipping.")
#             out_image = Image.open(source_path)
#             save_p = os.path.join(output_female, f"{video_name}_anon_nodet.png")
#             out_image.save(save_p)
#             continue
...
#         # 4.3) Prepare config for anonymization
#         conf = {
#             "image": frame_pil,
#             "prompt": "",
#             "mask": None,
...
#         # We'll merge results from each detected face
#         generated = []
#         dist = 0.0
#         attrs = []

#         # If you only want to anonymize the first face, break after the first
#         # Otherwise, remove the break to anonymize multiple faces
...
#         # 4.5) Build the final filename
#         dist_str = f"{dist:.3f}"
#         attrs_str = "_".join(a.replace(" ", "-") for a in attrs)
#         filename_info = f"{dist_str}_{attrs_str}" if attrs_str else dist_str

#         # e.g. "example_anon_0.345_young_man.png"
#         save_p = os.path.join(anon_output_folder, f"{video_name}_anon_{filename_info}.png")
#         out_image.save(save_p)
#         print(f"Saved anonymized image -> {save_p}")
