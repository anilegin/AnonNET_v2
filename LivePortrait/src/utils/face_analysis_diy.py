...
class FaceAnalysisDIY(FaceAnalysis):
    ...
    def get(self, img_bgr, **kwargs):
        ...
        if bboxes.shape[0] == 0:
            log("No face detected with InsightFace. Falling back to DeepFace RetinaFace...")
            return self._fallback_with_deepface(img_bgr, max_num, direction)
        ret = []
    ...
    def _fallback_with_deepface(self, img_bgr, max_num, direction):
        ...
        try:
            results = DeepFace.extract_faces(
                img_path=img_rgb,
                #target_size=(224, 224),  # size doesn't really matter since we're using just the bbox
                detector_backend="retinaface",
                enforce_detection=False,
                align=False,
            )
        except Exception as e:
            log(f"DeepFace fallback failed: {e}")
            return []

        faces = []
        for res in results[:max_num or None]:
            region = res.get("facial_area", None)
            if region is None:
                continue
            x, y, w, h = region["x"], region["y"], region["w"], region["h"]
            bbox = np.array([x, y, x + w, y + h])
            face = Face(bbox=bbox, kps=None, det_score=1.0)
            faces.append(face)
            
        if len(faces) == 0:
            log("No face detected with DeepFace RetinaFace fallback.")
            return []
