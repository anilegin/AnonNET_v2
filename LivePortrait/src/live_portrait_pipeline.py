...
class LivePortraitPipeline(object):
    ...
    def execute(self, args: ArgumentConfig):
        ...
        if flag_load_from_template:
            ...
            # wfp_template = remove_suffix(args.driving) + '.pkl' #MY CHANGE HERE THIS LINE WITH 
            #MY CHANGE START
            import os
            from pathlib import Path

            # 1. Extract the original video name
            base_name = os.path.basename(args.driving)      # e.g. "example.mp4"
            video_name = os.path.splitext(base_name)[0]
            
            output_dir = os.path.join("/data/stars/share/CelebV_HQ_Anon", "motion_templates")
            os.makedirs(output_dir, exist_ok=True)

            # 3. Create the new save path
            wfp_template = os.path.join(output_dir, f"{video_name}.pkl")
            #MY CHANGE END
